...
import com.organizaAi.OrganizaAi.infra.exceptions.BusinessException;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Object>> handleUnauthorized(UnauthorizedException ex) {
        // 403 quando é autorização (usuário autenticado, porém sem permissão)
        return build(HttpStatus.FORBIDDEN, new ApiError("403", ex.getMessage()), ex.getMessage());
    }

    @ExceptionHandler(BusinessException.class)
    public ResponseEntity<ApiResponse<Object>> handleBusiness(BusinessException ex) {
        return build(HttpStatus.BAD_REQUEST, new ApiError("400", ex.getMessage()), ex.getMessage());
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    public ResponseEntity<ApiResponse<Object>> handleIntegrity(DataIntegrityViolationException ex) {
        // Útil para violações de UNIQUE, FK, etc.
        return build(HttpStatus.CONFLICT, new ApiError("409", "Conflito de dados: " + mostSpecificMessage(ex)), "Conflito de dados.");
    }
    ...
    public ResponseEntity<ApiResponse<Object>> handleValidation(MethodArgumentNotValidException ex) {
        ...
        ApiError apiError = new ApiError("400", "Validação falhou");
        return build(HttpStatus.BAD_REQUEST, apiError, "Dados inválidos.");
    }
